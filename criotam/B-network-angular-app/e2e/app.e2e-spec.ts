...
describe('Starting tests for B-network-angular-app', function() {
  ...
  it('website title should be B-network-angular-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('B-network-angular-app');
    })
  });

  it('network-name should be criotam@0.0.2',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('criotam@0.0.2.bna');
    });
  });

  it('navbar-brand should be B-network-angular-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('B-network-angular-app');
    });
  });
